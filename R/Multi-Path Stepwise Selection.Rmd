...
This report demonstrates the `finalprojectgroup12` package that implements a multi-path forward stepwise selection using AIC, resampling-based stability estimation, and plausible model selection. We illustrate the workflow with a small simulated Gaussian example and a simulated logistic example, show variable stability results, and produce a final set of plausible models based on AIC tolerance and average stability thresholds.
...
remotes::install_github("R-4-Data-Science/rashomonlite10")

library(rashomonlite10)
