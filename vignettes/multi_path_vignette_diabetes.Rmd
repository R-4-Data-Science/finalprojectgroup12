...
if (p_base >= 2) {
  pairs     <- combn(seq_len(p_base), 2)   # all column index pairs
  num_int   <- ncol(pairs)
...
x        = X_train_model,
y        = y_train,
B        = 10,           # fewer resamples for vignette speed
resample = "bootstrap",
build_args = list(
family = "gaussian",
K      = 10,
...
## Requires: forest, X_train_model, y_train, X_test_model, y_test

# 1. Ensure training and test matrices are data frames
X_train_df <- as.data.frame(X_train_model)
X_test_df  <- as.data.frame(X_test_model)

# 2. Pick the best model by AIC
best_idx <- which.min(forest$aic_by_model$AIC)

# forest$models is assumed to be a list of fitted lm objects
best_model <- forest$models[[best_idx]]

if (is.null(best_model)) {
  stop("Best model object is NULL in forest$models.")
}

# 3. Extract variable names (drop intercept)
best_model_vars <- names(coef(best_model))
best_model_vars <- setdiff(best_model_vars, "(Intercept)")
...
# 6. Refit the model on the full training data
best_fit <- lm(form, data = train_df)

# 7. Prepare test data with the same predictors
if (length(best_model_vars) == 0) {
  test_df <- data.frame(X_test_df)[, 0, drop = FALSE]  # no predictors
} else {
  test_df <- X_test_df[, best_model_vars, drop = FALSE]
}
